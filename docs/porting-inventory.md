# Master v13 → V14 inventory

Originally auto-generated by a scratchpad `gen-tracker.py` (no longer on disk). Now
hand-maintained: flip a row's Status cell as you port it, or rebuild a generator from
`tools/v13-behavior-identifiers.json` + the `upstream/v13` pack data. See
[porting-status.md](porting-status.md) → *New-session bootstrap for legacy porting*.
